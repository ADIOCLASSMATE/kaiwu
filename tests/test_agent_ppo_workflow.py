...
def _load_workflow_module():
    ...
    feature_definition = types.ModuleType("agent_ppo.feature.definition")
    ...
    previous_modules["agent_ppo.feature.definition"] = _install_module(
    ...
    class _StubEnvConfManager:
        ...
        def extract_hero_ids_from_usr_conf(usr_conf):
            blue = [item["hero_id"] for item in usr_conf.get("lineups", {}).get("blue_camp", [])]
            red = [item["hero_id"] for item in usr_conf.get("lineups", {}).get("red_camp", [])]
            return blue, red
    ...
    try:
        path = Path(__file__).resolve().parents[1] / "agent_ppo/workflow/train_workflow.py"
        spec = importlib.util.spec_from_file_location("_agent_ppo_train_workflow_test", path)
...
class AgentPpoWorkflowTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _runner(self, monitor_side=0):
        ...
        runner.train_opponent_mix = {
            ...
            "common_ai": 0.6,
            "model_pool": 0.0,
            "low_win_selfplay": 0.3,
            "low_win_common_ai": 0.7,
    ...
    def test_training_config_enables_three_heroes_and_dynamic_opponent_mix(self):
        config_path = (
        ...
        self.assertEqual(
            [item["hero_id"] for item in config["lineups"]["blue_camp"]],
            [112, 133, 199],
        )
        self.assertEqual(
            [item["hero_id"] for item in config["lineups"]["red_camp"]],
            [112, 133, 199],
        )
        mix = config["episode"]["train_opponent_mix"]
        self.assertTrue(mix["enable"])
        self.assertTrue(mix["dynamic"])
        self.assertEqual(mix["selfplay"], 0.4)
        self.assertEqual(mix["common_ai"], 0.6)
        self.assertEqual(mix["low_win_selfplay"], 0.3)
        self.assertEqual(mix["low_win_common_ai"], 0.7)
    ...
    def test_dynamic_opponent_mix_uses_common_ai_win_rate_bands(self):
        ...
        self.assertEqual((low["selfplay"], low["common_ai"]), (0.3, 0.7))
        self.assertEqual((mid["selfplay"], mid["common_ai"]), (0.35, 0.65))
        self.assertEqual((high["selfplay"], high["common_ai"]), (0.5, 0.5))

    def test_lineup_metrics_identify_opponent_and_heroes(self):
        runner = self._runner(monitor_side=1)
        usr_conf = {
            "lineups": {
                "blue_camp": [{"hero_id": 112}],
                "red_camp": [{"hero_id": 199}],
            }
        }

        metrics = runner._episode_lineup_metrics(
            usr_conf,
            monitor_side=1,
            opponent_agent="common_ai",
        )

        self.assertEqual(metrics["opponent_common_ai"], 1.0)
        self.assertEqual(metrics["opponent_selfplay"], 0.0)
        self.assertEqual(metrics["hero_199"], 1.0)
        self.assertEqual(metrics["enemy_hero_112"], 1.0)
        self.assertEqual(metrics["mirror_199"], 0.0)
