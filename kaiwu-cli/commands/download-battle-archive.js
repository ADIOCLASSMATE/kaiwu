...
export default {
  ...
  run: async (kwargs) => {
    ...
    // 1. 触发 archive
    await api('/api/v5/Competition/ArchiveBattleTaskFile',
      withCtx(session, { id: battleTaskId, file_type: kind }), { session });
    ...
    while (Date.now() < deadline && !file) {
      const t = await api('/api/v5/Competition/GetBattleTask',
        withCtx(session, { id: battleTaskId }), { session });
